...
def fill_hole(mask):
    """
    fill_hole:
      Fills any holes in a contour for an image mask.
      
    Arguments:
      mask - image imported via skimage.io.imread
      
    Returns:
      Updated mask with the same dimensions as original image.
    """
...
def clean_mask(mask):
    """
    clean_mask:
      Fills any holes in an image mask, then finds
      contours for the largest-area segment.
      
    Arguments:
      mask - image imported via skimage.io.imread
      
    Returns:
      - Updated mask based on contours
      - The contours to create the updated mask (array of coordinate values)
    """
    mask = fill_hole(mask)
